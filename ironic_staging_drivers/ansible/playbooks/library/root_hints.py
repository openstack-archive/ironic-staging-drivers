...
def main():
    ...
    if devices_wwn is None:
        wwn_hints = set(hints) & EXTRA_PARAMS
        if wwn_hints:
            module.fail_json(msg='Extra hints is set but info is not '
                             'collected. Extra hints: %s' % wwn_hints)

    devices_info = merge_devices_info(devices, devices_wwn)
    hint = root_hint(hints, devices_info)

    if hint is None:
        module.fail_json(msg='Root hints is set but not proper device found. '
                         'Collected devices info: %s' % devices_info)
