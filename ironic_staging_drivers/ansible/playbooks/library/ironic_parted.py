...
---
module: parted
short_description: Create disk partiton tables and partitions
description: uses GNU parted utility
author: Pavlo Shchelokovskyy @pshchelo
version_added: null
notes:
- IS NOT IDEMPOTENT! partitons and table (if requested) are created anyway
...
options:
    ...
    label:
        description: |
            ...
            default does not create a new partition table
        required: false
        default: none
        choices: [none, msdos, gpt]
        aliases: []
        version_added: null
    commit:
        description: |
        ...
        description:|
            ...
            - size: <int>, required
              ...
              name: <str> (optional) name of the partition,
                    actual setting it is supported for gpt partitions only,
...
def validate_msdos(module, partitions):
    ...
    p_types = list([p['type'] for p in partitions])
    ...
    if 'extended' in p_types:
        ...
        # removed
        if list([k for k, g in itertools.groupby(p_types)]) != allowed:
            module.fail_json("Incorrect partitons order: for MSDOS, "
                             "all primary, single extended, all logical")
    elif 'logical' in p_types:
        # NOTE(pas-ha) logical has sense only with extended
        module.fail_json("Logical partiton w/o extended one on MSDOS "
                         "partition table")
...
def main():
    ...
    if module.check_mode or not commit:
        short_dev = device.split('/')[-1]
        created_partitions = {}
        for i, p in enumerate(partitions):
            created_partitions[p['name']] = '%s%s' % (short_dev, i + 1)
        module.exit_json(changed=not commit, created=created_partitions)
    ...
    if label:
        ...
        dev_cache = parse_lsblk_output(lsblk_output)
    ...
    for partition in partitions:
        ...
        dev_current = parse_lsblk_output(lsblk_output)
