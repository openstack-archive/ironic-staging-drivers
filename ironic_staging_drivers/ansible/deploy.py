...
ansible_opts = [
    ...
    cfg.BoolOpt('use_ramdisk_callback',
                ...
                help=_('Use callback request from ramdisk for start deploy or '
                       'cleaning.')),
    cfg.StrOpt('inspection_user',
               default='ansible',
               help=_('Inspect ramdisk user name is used by Ansible'
                      'This user must have passwordless sudo permissions. '
                      'Default is "ansible".')),
    cfg.StrOpt('inspection_key_file',
               help=_('Path to private key file for introspection ramdisk')),
]
...
REQUIRED_PROPERTIES = {
    ...
    'deploy_key_file': _('Path to private key file. If not specified, '
                         ...
                         'pre-loaded into ssh-agent. Optional.'),
    ...
    'inspect_playbook': _('Name of the Ansible playbook used for inspection. '
                        'Default is %s.') % DEFAULT_PLAYBOOKS['inspect'],
    'clean_steps_config': _('Name of the file with default cleaning steps '
                            'configuration. Default is %s. Optional'
                            ) % DEFAULT_CLEAN_STEPS
}
...
class AnsibleVendor(base.VendorInterface):
    ...
    @base.driver_passthru(['POST'])
    def inspect(self, context, **kwargs):
        try:
            callback_url = kwargs['callback_url']
            inspector_url = kwargs['inspector_url']
        except KeyError:
            raise exception.MissingParameterValue(
                _('For inspect operation, "callback_url" and '
                  '"inspector_url" must be specified.'))
        ...
        user = CONF.ansible.inspection_user
        key = CONF.ansible.inspection_key_file
        default_playbook = DEFAULT_PLAYBOOKS.get('inspect')
        # TODO(aarefiev): Use default inspection playbook, switch on node
        #  depended when ironic manage dhcp for introspection.
        ...
        extra_vars = {'inspector_url': inspector_url} 
        extra_vars = _prepare_extra_vars([(node_address, node_address, user)], extra_vars)
