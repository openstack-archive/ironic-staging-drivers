...
class AnsibleDeploy(agent_base.HeartbeatMixin, base.DeployInterface):
    ...
    def _ansible_deploy(self, task, node_address, async=True):
    ...
    def deploy(self, task):
        ...
        try:
            self._ansible_deploy(task, ip_addr, async=False)
        except Exception as e:
    ...
    @METRICS.timer('AnsibleDeploy.deploy_has_started')
    def deploy_has_started(self, task):
        return task.node.uuid in self._task_pool

    @METRICS.timer('AnsibleDeploy.deploy_is_done')
