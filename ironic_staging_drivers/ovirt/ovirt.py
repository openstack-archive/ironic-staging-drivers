...
from ironic_staging_drivers.common.exception import oVirtError
...
def _parse_driver_info(node):
    ...
    if d_info['vm_name'] is None:
        msg = _("Missing vm_name's in the node's driver_info")
...
def _getvm(driver_info):
    ...
    try:
        ...
        msg = ("Could not fetch information about VM %s"
               "from oVirt instance %s" % (name, url))
        raise oVirtError(msg)
    if vmsearch:
        return vms_service.vm_service(vmsearch[0].id)
    else:
        raise oVirtError(_("oVirt VM with name %s was not found") % name)
