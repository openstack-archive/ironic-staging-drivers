...
PROPERTIES = {
    ...
    'ovirt_insecure': _("Skips verification of the oVirt host certificate"),
...
def _parse_driver_info(node):
    ...
    for prop in PROPERTIES:
        ...
        value = next((e for e in [node_value, conf_value] if e is not None),
                     None)
        if value is None and prop != 'ovirt_ca_file':
            msg = _("Missing %(prop)s in the node's driver_info")
            raise exception.MissingParameterValue(msg.format(prop=prop))
        else:
...
class oVirtManagement(base.ManagementInterface):
    ...
    def get_boot_device(self, task):
        ...
        if not ironic_boot_dev:
            ...
            raise staging_exception.oVirtError(msg.format(boot_dev=boot_dev,
                                               node=task.node.uuid))
