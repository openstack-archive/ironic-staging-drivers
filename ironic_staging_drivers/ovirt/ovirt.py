...
opts = [
    ...
    cfg.StrOpt('insecure',
               default=False,
               help='oVirt insecure mode.\
               Skips verification of the oVirt host certificate'),
    cfg.StrOpt('ca_file',
...
def _parse_driver_info(node):
    ...
    :raises: InvalidParameterValue, if some parameter(s) have invalid value(s)
        in the node's driver_info.
    """
    d_info = {attr: getattr(CONF.ovirt, attr) for attr in CONF.ovirt}
    d_info['vm_name'] = node.driver_info.get('ovirt_vm_name')
    if d_info['vm_name'] is None:
        msg = _("Missing vm_name's information")
        raise exception.MissingParameterValue(msg)
    return d_info


def _getvm(driver_info):
    host, user = driver_info['host'], driver_info['user']
    password = driver_info['password']
    insecure = driver_info['insecure']
    ca_file = driver_info['ca_file']
    name = driver_info['vm_name']
    url = "https://%s/ovirt-engine/api" % host
    connection = sdk.Connection(url=url, username=user, password=password,
...
class oVirtPower(base.PowerInterface):
    ...
    def get_power_state(self, task):
        ...
        vm_name = driver_info['vm_name']
        vm = _getvm(driver_info)
        if vm is None:
            msg = "Ovirt couldn't find vm %(vm)s for node %(node)s"
            LOG.error(msg, {'node': task.node.uuid, 'vm': vm_name})
            return states.ERROR
        elif vm.get().status.value not in OVIRT_TO_IRONIC_POWER_MAPPING:
            msg = "oVirt returned unknown state for node %(node)s\
                  and vm %(vm)s"
            LOG.error(msg, {'node': task.node.uuid, 'vm': vm_name})
            return states.ERROR
        else:
            return OVIRT_TO_IRONIC_POWER_MAPPING[vm.get().status.value]
    ...
    def set_power_state(self, task, target_state, timeout=None):
        ...
        vm = _getvm(driver_info)
        if vm is None:
            raise exception.InvalidParameterValue('Invalid vm')
...
class oVirtManagement(base.ManagementInterface):
    ...
    def get_supported_boot_devices(self, task):
        ...
        """
        return list(IRONIC_TO_OVIRT_DEVICE_MAPPING)
    ...
    def get_boot_device(self, task):
        ...
        vm = _getvm(driver_info)
        if vm is None:
            msg = "oVirt returned unknown information for node %(node)s"
            LOG.error(msg, {'node': task.node.uuid})
            return
        ...
        if not ironic_boot_dev:
            persistent = False
            msg = "oVirt returned unknown boot device '%(device)s'\
                  for node %(node)s"
    ...
    def set_boot_device(self, task, device, persistent=False):
        ...
        try:
            ...
            LOG.error("'set_boot_device' failed for node %(node_id)s "
                      "with error: %(error)s",
