...
PROPERTIES = {
    'vm_name': _("Name of the VM in oVirt. Required."),
}
...
def _parse_driver_info(node):
    ...
    d_info['vm_name'] = node.driver_info.get('vm_name')
...
class oVirtPower(base.PowerInterface):
    ...
    def get_power_state(self, task):
        ...
        if vm is None:
            ...
            power_status = vm.get().status.value
        try:
            return OVIRT_TO_IRONIC_POWER_MAPPING[power_status]
        except KeyError:
            msg = "oVirt returned unknown state '%(state)s' for node %(node)s"
