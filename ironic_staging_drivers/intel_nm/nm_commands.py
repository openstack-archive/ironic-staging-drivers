...
def _handle_parsing_error(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        def _log_and_raise(msg, data):
            message = _('Intel Node Manager error: %s') % msg
            LOG.error(_LE('%(error)s, returned data: %(data)s'),
                      {'error': message, 'data': data})
            raise exception.IPMIFailure(message)

        try:
            return func(*args, **kwargs)
        except (IndexError, struct.error):
            _log_and_raise(_('wrong lenght of returned data'), args[0])
        except KeyError:
            _log_and_raise(_('corrupted data returned'), args[0])
        except ValueError:
            _log_and_raise(_('cannot convert returned data'), args[0])
...
def _raw_to_int(raw_data):
    """Converting list of raw hex valies as strings to integers."""
...
def set_policy(policy):
    ...
    enable = 0x10 if policy['enable'] else 0x00
    ...
    _append_to_command(cmd, _hex(INTEL_NM_TRIGGERS[policy['policy_trigger']] |
                       ...
                       | INTEL_NM_STORAGE[policy['storage']] | add_policy))
    ...
    if isinstance(policy['target_limit'], int):
        ...
        cores_disabled = policy['target_limit']['cores_disabled'] * 2
