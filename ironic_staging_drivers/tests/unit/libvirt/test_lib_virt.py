...
from ironic_staging_drivers.libvirt import lib_virt
...
class FakeLibvirtDomain(object):
    ...
    def XMLDesc(self, boot_dev=lib_virt._BOOT_DEVICES_MAP[boot_devices.PXE]):
        return(
...
class LibvirtValidateParametersTestCase(db_base.DbTestCase):
    ...
    def test__parse_driver_info_good_sasl_cred(self):
        ...
        self.assertIsNotNone(info.get('libvirt_uri'))
        self.assertIsNotNone(info.get('sasl_username'))
        self.assertIsNotNone(info.get('sasl_password'))
        self.assertIsNotNone(info.get('uuid'))
...
class LibvirtPrivateMethodsTestCase(db_base.DbTestCase):
    ...
    @mock.patch.object(lib_virt.libvirt, 'openAuth', autospec=True)
    def test__get_libvirt_connection_sasl_auth(self, libvirt_open_mock):
        node = obj_utils.get_test_node(
            self.context,
            driver='fake_libvirt_fake',
            driver_info=_get_test_libvirt_driver_info('sasl'))
        lib_virt._get_libvirt_connection(node['driver_info'])

        libvirt_open_mock.assert_called_once_with(
            'test+tcp://localhost:5000/test',
            [[lib_virt.libvirt.VIR_CRED_AUTHNAME,
              lib_virt.libvirt.VIR_CRED_PASSPHRASE],
             mock.ANY,  # Inline cred function
             None], 0)

    @mock.patch.object(lib_virt.libvirt, 'open', autospec=True)
    ...
    @mock.patch.object(lib_virt.libvirt, 'open',
                       side_effect=lib_virt.libvirt.libvirtError('Error'))
    def test__get_libvirt_connection_error_conn(self, libvirt_open_mock):
        ...
        self.assertRaises(isd_exc.LibvirtError,
                          lib_virt._get_libvirt_connection,
                          node['driver_info'])
    ...
    @mock.patch.object(lib_virt, '_get_libvirt_connection',
    ...
    def test__get_domain_by_macs(self, libvirt_conn_mock):
        ...
        with task_manager.acquire(self.context, node.uuid,
                                  shared=True) as task:
            domain = lib_virt._get_domain_by_macs(task)
    ...
    def test__get_power_state_error(self):
        ...
        domain_mock.isActive = mock.MagicMock(
            side_effect=lib_virt.libvirt.libvirtError('Test'))

        self.assertRaises(isd_exc.LibvirtError,
                          lib_virt._get_power_state,
                          domain_mock)

    @mock.patch.object(lib_virt, '_power_off', autospec=True)
    @mock.patch.object(lib_virt, '_power_on', return_value=states.POWER_ON)
    def test__power_cycle(self, power_on_mock, power_off_mock):
        lib_virt._power_cycle('fake domain')

        power_on_mock.assert_called_once_with('fake domain')
        power_off_mock.assert_called_once_with('fake domain')

    @mock.patch.object(lib_virt, '_power_off', autospec=True)
    @mock.patch.object(lib_virt, '_power_on', return_value=states.POWER_OFF)
    def test__power_cycle_failure(self, power_on_mock, power_off_mock):
        self.assertRaises(exception.PowerStateFailure,
                          lib_virt._power_cycle,
                          'fake domain')
        power_off_mock.assert_called_once_with('fake domain')

    @mock.patch.object(lib_virt, '_power_off', autospec=True)
    @mock.patch.object(lib_virt, '_power_on',
                       side_effect=lib_virt.libvirt.libvirtError('Test'))
    ...
    @mock.patch.object(lib_virt, '_get_power_state',
                       return_value=states.POWER_ON)
    def test__power_on_on(self, get_power_mock):
        state = lib_virt._power_on('fake domain')
    ...
    def test__power_on_off(self, get_power_mock):
        ...
        state = lib_virt._power_on(domain_mock)
    ...
    def test__power_off_off(self, get_power_mock):
        state = lib_virt._power_off('fake domain')
    ...
    def test__power_off_on(self, get_power_mock):
        ...
        state = lib_virt._power_off(domain_mock)
    ...
    def test__get_boot_device(self):
        ...
        boot_dev = lib_virt._get_boot_device(domain)

        self.assertEqual(lib_virt._BOOT_DEVICES_MAP[boot_devices.PXE],
                         boot_dev)
    ...
    def test__set_boot_device(self):
        ...
        lib_virt._set_boot_device(
            conn, domain, lib_virt._BOOT_DEVICES_MAP[boot_devices.DISK])
    ...
    def test__set_boot_device_error(self):
        ...
        self.assertRaises(isd_exc.LibvirtError,
                          lib_virt._set_boot_device,
                          conn, domain,
                          lib_virt._BOOT_DEVICES_MAP[boot_devices.DISK])
...
class LibvirtPowerTestCase(db_base.DbTestCase):
    ...
    @mock.patch.object(driver_utils, 'get_node_mac_addresses', autospec=True)
    def test_validate_get_macs_error(self, get_node_macs_mock):
        get_node_macs_mock.return_value = None
    ...
    @mock.patch.object(lib_virt.driver_utils,
    ...
    @mock.patch.object(lib_virt, '_get_power_state', autospec=True)
    @mock.patch.object(lib_virt, '_get_domain_by_macs', autospec=True)
    ...
    @mock.patch.object(lib_virt, '_power_on', autospec=True)
    @mock.patch.object(lib_virt, '_get_domain_by_macs', autospec=True)
...
class LibvirtManagementTestCase(db_base.DbTestCase):
    ...
    @mock.patch.object(lib_virt, '_parse_driver_info', autospec=True)
    ...
    @mock.patch.object(lib_virt, '_get_domain_by_macs',
    ...
    @mock.patch.object(lib_virt, '_get_boot_device', return_value=None)
    @mock.patch.object(lib_virt, '_get_domain_by_macs', autospec=True)
    ...
    @mock.patch.object(lib_virt, '_set_boot_device', autospec=True)
    @mock.patch.object(lib_virt, '_get_libvirt_connection', autospec=True)
    @mock.patch.object(lib_virt, '_get_domain_by_macs', autospec=True)
    def test_set_boot_device_ok(self, get_domain_mock, get_conn_mock,
        ...
        with task_manager.acquire(self.context, self.node.uuid,
            ...
            set_boot_dev_mock.assert_called_once_with(
                ...
                lib_virt._BOOT_DEVICES_MAP[boot_devices.PXE])
