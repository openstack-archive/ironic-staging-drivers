...
@mock.patch.object(commands.LOG, 'error', autospec=True)
class ParsingErrorDecoratorTestCase(testtools.TestCase):

    def test_parse_no_errors(self, mock_log):
        self.assertEqual('foo', fake_parse('foo'))
        self.assertFalse(mock_log.called)

    def test_parse_handled_exception(self, mock_log):
        self.assertRaises(exception.IPMIFailure, fake_parse_exc, 'foo')
        self.assertTrue(mock_log.called)


class IntelNMPoliciesCommandTestCase(testtools.TestCase):
...
class ParsingFromFileTestCase(testtools.TestCase):
    ...
    def test_parsing_found(self):
        ...
        self.assertEqual(('0x6a', '0xb'), result)
