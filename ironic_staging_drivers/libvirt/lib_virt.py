...
def _get_power_state(domain):
    ...
    try:
        ...
        raise ir_exc.LibvirtError(err=e)
...
def _get_boot_device(domain):
    ...
    parsed = ET.fromstring(domain.XMLDesc()).find('.//os/boot')
    boot_dev = None
...
class LibvirtPower(base.PowerInterface):
    ...
    def validate(self, task):
       ...
       """
        ...
        except isd_exc.LibvirtError:
            LOG.error(_LE("Failed to get libvirt connection node %(node)s"),
                      {'node': task.node.uuid})
            raise ir_exc.InvalidParameterValue(_("Libvirt connection cannot"
                                                 " be established"))
