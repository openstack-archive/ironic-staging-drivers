...
function set_ansible_deploy_driver {
    ...
    # NOTE(pas-ha) we can do it this late as this config is read
    # by our custom Ansible callback plugin only when we really start
    # ansible-playbook process during provisioning
